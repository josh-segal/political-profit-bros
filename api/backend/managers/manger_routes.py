...
def get_politician_portfolio(manager_id):
    ...
    # use cursor to query the database for a list of politicians
    cursor.execute(f"SELECT DISTINCT p.name, p.party, p.state, pm.manager_id, p.id, pm.candidate_opp FROM politician p JOIN politician_manager pm ON p.id = pm.politician_id WHERE pm.manager_id = '{manager_id}'")
